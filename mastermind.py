...
def hard(): #Klar
    return [str(randint(1, 6)) for _ in range(4)]
...
def gui(guesses, feedback):
    ...
    for i in range(8):
        print(f"\n{Colors.text}{(8 - i):5.0f}         {guesses[8 - (i + 1)]}         {feedback[8 - (i + 1)]}{Colors.endc}")
    #Skriver ut talen från 8 till 1 och sen feedbacken för just den omgången anropas och skrivs ut
...
def guess():
    ...
    while True:
        ...
        amount_guesses += 1
        #man kan bara gissa 8 gånger sen är spelet över

def info():
    kört_innan = input("Do you require information (yes/no) -> ")
    while kört_innan not in ["yes", "no", "y", "n"]:
        #När inputcheck() returnerar True så avbryts loopen
        if kört_innan not in ["yes", "no", "y", "n"]:
...
def play():
    if info() is True:
        ...
        Your goal is to guess the code in 8 guesses
